...
public class CourseDiscussionCommentsFragment extends BaseFragment implements DiscussionCommentsAdapter.Listener {
    ...
    @InjectView(com.nile.kmooc.R.id.discussion_recycler_view)
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        return inflater.inflate(com.nile.kmooc.R.layout.fragment_discussion_responses_or_comments, container, false);
    }
    ...
    public void onViewCreated(View view, Bundle savedInstanceState) {
        ...
        final int overlap = getResources().getDimensionPixelSize(com.nile.kmooc.R.dimen.edx_hairline);
        ...
        DiscussionUtils.setStateOnTopicClosed(discussionThread.isClosed(),
                createNewCommentTextView, com.nile.kmooc.R.string.discussion_post_create_new_comment,
                com.nile.kmooc.R.string.discussion_add_comment_disabled_title, createNewCommentLayout,
    ...
    public void onEventMainThread(DiscussionCommentPostedEvent event) {
        if (null != event.getParent() && event.getParent().getIdentifier().equals(discussionResponse.getIdentifier())) {
            ((BaseFragmentActivity) getActivity()).showInfoMessage(getString(com.nile.kmooc.R.string.discussion_comment_posted));
            if (!hasMorePages) {
    ...
    public void reportComment(@NonNull DiscussionComment comment) {
        setCommentFlaggedCall = discussionService.setCommentFlagged(
                comment.getIdentifier(), new DiscussionService.FlagBody(!comment.isAbuseFlagged()));
