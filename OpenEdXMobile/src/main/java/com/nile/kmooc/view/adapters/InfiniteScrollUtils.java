...
public class InfiniteScrollUtils {
    ...
    public static <T> ListContentController<T> configureListContentController(@NonNull ListView list, @NonNull final ArrayAdapter<T> adapter) {
        final View footerView = LayoutInflater.from(list.getContext()).inflate(com.nile.kmooc.R.layout.list_view_footer_progress, list, false);
        list.addFooterView(footerView, null, false);
        final View loadingIndicator = footerView.findViewById(com.nile.kmooc.R.id.loading_indicator);
