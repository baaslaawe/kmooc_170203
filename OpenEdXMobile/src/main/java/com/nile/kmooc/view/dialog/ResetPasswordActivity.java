...
import com.nile.kmooc.databinding.ActivityResetPasswordBinding;
...
public class ResetPasswordActivity extends BaseAppActivity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        binding = DataBindingUtil.setContentView(this, com.nile.kmooc.R.layout.activity_reset_password);
    ...
    private void onSubmit() {
        if (!NetworkUtil.isConnected(getApplicationContext())) {
            onResetError(getString(com.nile.kmooc.R.string.network_not_connected_short));
            return;
        }
        final String email = binding.emailEdit.getText().toString().trim();
        if (!InputValidationUtil.isValidEmail(email)) {
            binding.emailEdit.requestFocus();
            onResetError(getString(com.nile.kmooc.R.string.error_invalid_email));
            return;
        }
