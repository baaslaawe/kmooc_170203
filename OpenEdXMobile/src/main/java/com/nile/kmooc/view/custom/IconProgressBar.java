...
public class IconProgressBar extends ProgressBar {
    ...
    public IconProgressBar(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        final TypedArray a = context.obtainStyledAttributes(
                attrs, com.nile.kmooc.R.styleable.IconProgressBar, defStyleAttr, 0);
        ColorStateList colorStateList = a.getColorStateList(
                com.nile.kmooc.R.styleable.IconProgressBar_indeterminateIconColor);
        this.colorStateList = colorStateList != null ? colorStateList :
                ColorStateList.valueOf(DEFAULT_COLOR);
        String iconKey = a.getString(com.nile.kmooc.R.styleable.IconProgressBar_indeterminateIconName);
        if (iconKey != null) {
            IconDrawable drawable = new IconDrawable(context, iconKey);
            if (a.getBoolean(com.nile.kmooc.R.styleable.IconProgressBar_indeterminateIconPulse, false)) {
