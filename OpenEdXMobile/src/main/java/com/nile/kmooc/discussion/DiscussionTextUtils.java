...
public abstract class DiscussionTextUtils {
    ...
    public static void setAuthorAttributionText(@NonNull TextView textView,
        ...
        final CharSequence text;
        {
            ...
            switch (authorAttributionLabel) {
                case ANSWER:
                    isEndorsed = true;
                    joinableStrings.add(context.getString(com.nile.kmooc.R.string.discussion_post_marked_as_answer));
                    break;
                case ENDORSEMENT:
                    isEndorsed = true;
                    joinableStrings.add(context.getString(com.nile.kmooc.R.string.discussion_post_endorsed));
                    break;
            }
            ...
            if (!TextUtils.isEmpty(author)) {
                ...
                if (config.isUserProfilesEnabled() && !authorData.isAuthorAnonymous()) {
                    ...
                    authorSpan.setSpan(new ForegroundColorSpan(context.getResources().
                                    getColor(com.nile.kmooc.R.color.edx_brand_primary_base)), 0,
                ...
                joinableStrings.add(ResourceUtil.getFormattedString(context.getResources(),
                        com.nile.kmooc.R.string.discussion_post_author_attribution, "author", authorSpan));
            }

            final String authorLabel = authorData.getAuthorLabel();
            if (!TextUtils.isEmpty(authorLabel)) {
                joinableStrings.add(ResourceUtil.getFormattedString(context.getResources(),
                        com.nile.kmooc.R.string.discussion_post_author_label_attribution, "text", authorLabel));
            }
    ...
    public static CharSequence getRelativeTimeSpanString(@NonNull Context context, long nowMs,
        ...
        if (nowMs - timeMs < DateUtils.SECOND_IN_MILLIS) {
            return context.getString(com.nile.kmooc.R.string.just_now);
        } else {
    ...
    public static void setEndorsedState(@NonNull TextView target,
        ...
        if (response.isEndorsed()) {
            switch (thread.getType()) {
                case QUESTION:
                    target.setText(com.nile.kmooc.R.string.discussion_responses_answer);
                    break;
                case DISCUSSION:
                default:
                    target.setText(com.nile.kmooc.R.string.discussion_responses_endorsed);
                    break;
            }
