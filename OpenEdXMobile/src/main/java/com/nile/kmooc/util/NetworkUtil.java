    ...
    public static boolean verifyDownloadPossible(BaseFragmentActivity activity) {
        if (!new PrefManager(activity, PrefManager.Pref.WIFI).getBoolean(PrefManager.Key
            ...
            if (!isConnectedWifi(activity)) {
                activity.showInfoMessage(activity.getString(com.nile.kmooc.R.string.wifi_off_message));
                return false;
            }
        } else {
            if (!isConnected(activity)) {
                activity.showInfoMessage(activity.getString(com.nile.kmooc.R.string.network_not_connected));
                return false;
            }
        }
        return true;
